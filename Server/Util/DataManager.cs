						...
						try {
							if (c.dataRemaining()) {
								...
								if (p.getType().Equals(PacketType.LOGIN)) {
									...
									MySqlDataReader r = Server.getSingleton().getDbManager().prepareCommand("select `id` from ?UserTable where `username`=?User and `pass`=?Password;", KVFactory.obj("?User", packet.getUser()), KVFactory.obj("?Password", packet.getPasswordHash())).ExecuteReader();
									if (r.Read()) {
									...
									MySqlDataReader reader = Server.getSingleton().getDbManager().prepareCommand("select * from ?PlayerTable where `serverId`=(select `serverId` from `servers` where `serverName`=?Server) and `playerName` like ?Name;", KVFactory.obj("?Server", packet.getServer()), KVFactory.obj("?Name", packet.getName())).ExecuteReader();
									if (reader.Read()) {
										...
										MySqlDataReader r = Server.getSingleton().getDbManager().prepareCommand("select `serverId` from ?ServerTable where `serverName`=?Server", KVFactory.obj("?Server", packet.getServer())).ExecuteReader();
										r.Read();
										...
										MySqlDataReader z = Server.getSingleton().getDbManager().prepareCommand("select `id`,`serverId` from ?PlayerTable where `serverId`=" + serverId + " and `playerName`=\"" + packet.getName() + "\"").ExecuteReader();
										z.Read();
									...
									MySqlDataReader reader = Server.getSingleton().getDbManager().executeReader("select * from ?ServerTable");
									while (reader.Read())
									...
									MySqlDataReader r = Server.getSingleton().getDbManager().prepareCommand("select `screenshotId`,`uploadDate`,`uploadingUserId` from ?AttachmentTable where `playerId`=?PlayerId and `serverId`=?ServerId",
									...
									MySqlCommand cmd = Server.getSingleton().getDbManager().prepareCommand("insert into ?AttachmentTable (`playerId`, `serverId`, `data`, `uploadDate`, `uploadingUserId`) values(?playerId, ?serverId, ?attachmentData, ?dateTime, ?userId)",
										KVFactory.obj("?PlayerId", packet.getPlayerId()),
									...
									MySqlDataReader r = Server.getSingleton().getDbManager().executeReader("select `data` from ?AttachmentTable where `screenshotId`=" + packet.getId());
									r.Read();
